    ...
    <section id="how-it-works" className="bg-background py-24 sm:py-32">
      <Container>
        <Reveal className="mx-auto max-w-2xl text-center">
          <span className="text-sm font-medium tracking-wide text-gold-600">{t.howItWorks.eyebrow}</span>
        ...
        <div className="mt-16 grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-4">
          {t.howItWorks.steps.map((step, index) => (
            <Reveal key={step.title} delay={index * 80} className="relative">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-ink-900 font-heading text-xl text-gold-300">
                {index + 1}
              </div>
              <h3 className="mt-5 font-heading text-lg text-ink-900">{step.title}</h3>
